...
class BLEService extends ChangeNotifier {
  ...
  Future<bool> connect(BluetoothDevice device) async {
    try {
      ...
      // Debug: afficher tous les services trouvés
      debugPrint('📋 Services trouvés: ${services.length}');
      for (var s in services) {
        debugPrint('  Service: ${s.uuid.toString().toLowerCase()}');
      ...
      for (BluetoothService service in services) {
        ...
        // Comparer UUID court ou long
        if (serviceUuid == _serviceScoreUUID || serviceUuid.contains('aa00')) {
          debugPrint('✅ Service Score trouvé');
          for (BluetoothCharacteristic char in service.characteristics) {
            final uuid = char.uuid.toString().toLowerCase();
            
            if (uuid == _charScoreUpdateUUID || uuid.contains('aa01')) {
              _scoreUpdateChar = char;
              debugPrint('  ✅ ScoreUpdate char trouvée');
              await _subscribeToScoreUpdates(char);
            } else if (uuid == _charCommandUUID || uuid.contains('aa02')) {
              _commandChar = char;
              debugPrint('  ✅ Command char trouvée');
            } else if (uuid == _charMatchStatusUUID || uuid.contains('aa03')) {
              _matchStatusChar = char;
              debugPrint('  ✅ MatchStatus char trouvée');
              await _subscribeToMatchStatus(char);
            } else if (uuid == _charBatteryInfoUUID || uuid.contains('aa04')) {
              _batteryInfoChar = char;
              debugPrint('  ✅ BatteryInfo char trouvée');
              await _subscribeToBatteryInfo(char);
            }
          }
        } else if (serviceUuid == _servicePlayerUUID || serviceUuid.contains('bb00')) {
          debugPrint('✅ Service Player trouvé');
          for (BluetoothCharacteristic char in service.characteristics) {
            final uuid = char.uuid.toString().toLowerCase();
            if (uuid == _charPlayerNamesUUID || uuid.contains('bb01')) {
              _playerNamesChar = char;
              debugPrint('✅ PlayerNames char trouvée');
            }
          }
        }
      }
      
      // Debug: résumé des caractéristiques trouvées
      debugPrint('📊 Caractéristiques trouvées:');
      debugPrint('  Score Update: ${_scoreUpdateChar != null}');
      debugPrint('  Command: ${_commandChar != null}');
      debugPrint('  Match Status: ${_matchStatusChar != null}');
      debugPrint('  Battery Info: ${_batteryInfoChar != null}');
      debugPrint('  Player Names: ${_playerNamesChar != null}');
      ...
      char.value.listen((value) {
        if (value.isNotEmpty) {
          try {
            ...
            debugPrint('⚠️  Erreur parsing score: $e');
          }
        }
      })
    );
          ...
          try {
            ...
            if (parts.length == 4) {
              final batteryInfo = BatteryInfo(
                systemVoltage: double.tryParse(parts[0]) ?? 0.0,
                systemPercent: int.tryParse(parts[1]) ?? 0,
                eTag1Percent: int.tryParse(parts[2]) ?? -1,
                eTag2Percent: int.tryParse(parts[3]) ?? -1,
              );
              _batteryStreamController.add(batteryInfo);
            }
          } catch (e) {
            debugPrint('⚠️  Erreur parsing batterie: $e');
          }
        }
      })
    );
